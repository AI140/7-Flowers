...
Scene* grammarScene2::createScene()
{
    return grammarScene2::create();
}
...
bool grammarScene2::init() {
    ...
    listener->onTouchBegan = CC_CALLBACK_2(grammarScene2::onTouchBegan, this);
    Director::getInstance()->getEventDispatcher()->addEventListenerWithSceneGraphPriority(listener, this);

    // TMXTiledMap::create()
    // 加载TMX瓦片地图
    TMXTiledMap* tileMap = TMXTiledMap::create("summer2.tmx");
    ...
    //设置上、左、右转向按钮
    auto forwardTurn = MenuItemImage ::create("forward.png","forward.png",CC_CALLBACK_1(grammarScene2::forwardTurnCallback,this));
    forwardTurn->setPosition(Vec2(visibleSize.width*3/4+20,visibleSize.height));

    auto leftTurn = MenuItemImage ::create("left.png","left.png",CC_CALLBACK_1(grammarScene2::leftTurnCallback,this));
    leftTurn->setPosition(Vec2(visibleSize.width*3/4+70,visibleSize.height));
    auto rightTurn = MenuItemImage ::create("right.png","right.png",CC_CALLBACK_1(grammarScene2::rightTurnCallback,this));
    rightTurn->setPosition(Vec2(visibleSize.width*3/4+120,visibleSize.height));

    //设置if按钮
    auto ifButton= MenuItemImage::create("if.png","if.png",CC_CALLBACK_1(grammarScene2::ifButtonCallback,this));
    ifButton->setPosition(Vec2(visibleSize.width*3/4+20,visibleSize.height-50));
    ifButton->setScale(1.8);

    auto menu3 = Menu::create(forwardTurn,leftTurn,rightTurn,ifButton,NULL);
    ...
    */
    ...
    person->setPosition(visibleSize.width/2+40,60);
    person->setTag(1);
    person->setScale(0.3);
    ...
    auto backItem = MenuItemImage::create("back.png", "back.png",
                                          CC_CALLBACK_1(grammarScene2::backToCallback, this));
    backItem->setScale(3);
    ...
    //定时器更新 每一秒刷新一次 更新倒计时
    this->schedule(schedule_selector(grammarScene2::update), 1.0f );

    //添加故事区
    CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("music/type.wav", false);
    auto story_text = Label::createWithTTF("\t\t“咦，只过了一天，丛林的季节就变化了吗？真是神奇！”\n\t赛文感叹着，加快脚步却突然发现情况有变：路上好像有些障碍。","fonts/STKAITI.TTF", 15);
...
void grammarScene2::backToCallback(Ref *pSender) {
    auto HelloWorldScene = HelloWorld::createScene();
    Director::getInstance()->replaceScene(HelloWorldScene);
    //两个类互相调用 要把主页面里的B变成指针
}

bool grammarScene2::onTouchBegan(Touch *touch, Event *unused_event) {
...
void grammarScene2::ifButtonCallback(Ref* pSender){
...
void grammarScene2::forwardTurnCallback(Ref* pSender){
    Sprite* person=(Sprite *)this->getChildByTag(1);
    Sprite* reward2=(Sprite *)this->getChildByName("reward2");
    ...
    if(step==8){
        ...
        popLayer->nextScene(5);
        popLayer->backScene(0);
        popLayer->setReword(reward2);
...
void grammarScene2::forwardTurn(){
    Sprite* person=(Sprite *)this->getChildByTag(1);
    Sprite* reward2=(Sprite *)this->getChildByName("reward2");
...
void grammarScene2::leftTurnCallback(Ref* pSender){
    Sprite* person=(Sprite *)this->getChildByTag(1);
...
void grammarScene2::rightTurnCallback(Ref* pSender){
    Sprite* person=(Sprite *)this->getChildByTag(1);
...
void grammarScene2::update(float dt){
